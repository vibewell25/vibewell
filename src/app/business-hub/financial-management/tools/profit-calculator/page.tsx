...
export default function ProfitCalculatorPage() {
  ...
  const [hourlyOverhead, setHourlyOverhead] = useState<number>(15);
  ...
  const addExpense = (category: 'direct' | 'indirect') => {
    setServices(services.map(service => {
      if (service.id === activeServiceId) {
        const newExpenseId = (Math.max(...service.expenses.map(e => parseInt(e.id)), 0) + 1).toString();
        return {
          ...service,
          expenses: [
            ...service.expenses,
  ...
  const deleteExpense = (expenseId: string) => {
    setServices(services.map(service => {
      if (service.id === activeServiceId) {
        return {
          ...service,
          expenses: service.expenses.filter(expense => expense.id !== expenseId)
        };
      }
      return service;
    }));
