...
import { detectDevicePerformanceProfile, isMobileDevice, applyDeviceClasses } from './deviceDetection';
 ...
 */
export function applyLazyLoading(maxImageWidth: number, deviceProfile: DevicePerformanceProfile): void {
  ...
  imageDomains.forEach(domain => {
    if (html.includes(domain) && !document.querySelector(`link[rel="preconnect"][href*="${domain}"]`)) {
...
export function applyEventThrottling(deviceProfile: DevicePerformanceProfile): void {
  ...
  const throttleTime = deviceProfile.type === 'low' ? 100 : 
                      deviceProfile.type === 'medium' ? 50 : 16;
