...
if (process.env.NODE_ENV === 'development' && messagesStore.conversations.length === 0) {
  ...
  const conversation1 = messagesStore.getOrCreateConversation(
    currentUserId, 'Current User', '/avatar-current.png',
    user1Id, 'Emma Thompson', '/avatar1.png'
  );
  
  // Add messages to conversation 1
  messagesStore.addMessage(conversation1.id, user1Id, "Hi there! I saw your post about meditation. I've been practicing for years and would love to share some tips!");
  messagesStore.addMessage(conversation1.id, currentUserId, "That would be amazing! I'm just getting started and could use some guidance.");
  messagesStore.addMessage(conversation1.id, user1Id, "Great! I recommend starting with just 5 minutes a day and gradually increasing. Consistency is more important than duration.");
  
  // Create conversation 2
  const conversation2 = messagesStore.getOrCreateConversation(
    currentUserId, 'Current User', '/avatar-current.png',
    user2Id, 'David Chen', '/avatar2.png'
  );
  
  // Add messages to conversation 2
  messagesStore.addMessage(conversation2.id, user2Id, "Hey! Are you joining the yoga challenge next week?");
  
  // Create conversation 3
  const conversation3 = messagesStore.getOrCreateConversation(
    currentUserId, 'Current User', '/avatar-current.png',
    user3Id, 'Sarah Williams', '/avatar3.png'
  );
  
  // Add messages to conversation 3
  messagesStore.addMessage(conversation3.id, user3Id, "Thanks for the nutrition advice! I tried that recipe and it was delicious.");
  messagesStore.addMessage(conversation3.id, user3Id, "Do you have any other healthy meal prep suggestions?");
}
