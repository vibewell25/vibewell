...
async function {
  ...
  if (Date.now() - start > 30000) throw new Error('Timeout'); testNVDA() {
...
console.log('Basic navigation:', results);
    ...
    const report = `# Screen Reader Test Results - NVDA
...
`;
    ...
    fs.writeFileSync('./accessibility/reports/nvda-test-results.md', report);
async function {
  const start = Date.now();
...
`;
    ...
    fs.appendFileSync('./accessibility/reports/nvda-test-results.md', formReport);
async function {
  const start = Date.now();
...
`;
    ...
    fs.appendFileSync('./accessibility/reports/nvda-test-results.md', dynamicReport);
// Run the tests
testNVDA().catch(console.error); 