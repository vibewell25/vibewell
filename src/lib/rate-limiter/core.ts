...
export async function logRateLimitEvent(
  ...
  try {
    ...
    const suspicious = !result.success || 
                      (result.remaining < result.limit * 0.1); // Less than 10% remaining
    
    // Calculate how far over the limit we are (for monitoring abuse)
    const overLimitFactor = result.success ? 0 : Math.min(5, Math.ceil(1 + (-result.remaining) / result.limit));
    ...
    if (suspicious || !result.success) {
      logger.info(`Rate limit ${result.success ? 'approaching' : 'exceeded'}: ${ip} on ${path}`, 'ratelimit', {
        ip,
        path,
        exceeded: !result.success,
        remaining: result.remaining,
