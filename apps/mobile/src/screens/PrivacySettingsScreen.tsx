...
const PrivacySettingsScreen: React.FC = () => {
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert(
                'Error',
                'There was a problem deleting your data. Please try again or contact support.'
              );
            } finally {
              setLoading(false);
            }
  ...
  // Function to delete user data from server
  const deleteUserData = async (): Promise<void> => {
    try {
      ...
      // Make API request to delete user data
      const response = await fetch('https://api.vibewell.com/v1/user/data', {
        method: 'DELETE',
        headers: {
      ...
      await AsyncStorage.removeItem('@vibewell/auth_token');
      
      return;
    } catch (error) {
      console.error('API error when deleting user data:', error);
      throw error;
    }
  };
