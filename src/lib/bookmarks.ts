...
const BOOKMARKS_STORAGE_KEY = 'vibewell_bookmarks';
const RECENT_VIEWS_STORAGE_KEY = 'vibewell_recent_views';
...
export function addBookmark(bookmark: Omit<Bookmark, 'timestamp'>): Bookmark {
  ...
  try {
    ...
    const existingIndex = bookmarks.findIndex(b => b.id === bookmark.id && b.type === bookmark.type);
...
export function trackRecentView(item: Omit<Bookmark, 'timestamp'>): void {
  ...
  try {
    ...
    if (existingIndex >= 0) {
      ...
      recentViews[existingIndex].viewCount += 1;
    } else {
      // Add new view
      recentViews.push({
        ...item,
