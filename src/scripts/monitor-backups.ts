...
async function monitorBackups() {
  ...
  try {
    ...
    logger.error('Error in backup monitoring:', error instanceof Error ? error.message : String(error));
    process.exit(1);
  }
}
...
monitorBackups()
  ...
  .catch((error) => {
    logger.error('Unhandled error in backup monitoring script:', error instanceof Error ? error.message : String(error));
    process.exit(1);
  }); 