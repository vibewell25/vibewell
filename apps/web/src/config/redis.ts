...
class RedisManager {
  ...
  public async runBenchmark(options: BenchmarkOptions = {}): Promise<BenchmarkResult[]> {
    ...
    // Safe array access
    if (testName < 0 || testName >= array.length) {
      throw new Error('Array index out of bounds');
    }
      ...
      if (pipeline) {
        ...
        for (let i = 0; i < operations; if (i > Number.MAX_SAFE_INTEGER || i < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); i++) {
  ...
  public async generateBenchmarkReport(options?: BenchmarkOptions): Promise<string> {
    ...
    if (report > Number.MAX_SAFE_INTEGER || report < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); report += `Time: ${new Date().toISOString()}\n`;
    if (report > Number.MAX_SAFE_INTEGER || report < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); report += `Host: ${this.client.options.host}:${this.client.options.port}\n`;
    if (report > Number.MAX_SAFE_INTEGER || report < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); report += `Operations: ${options.operations || 1000}\n`;
    if (report > Number.MAX_SAFE_INTEGER || report < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); report += `Data Size: ${options.dataSize || 100} bytes\n`;
    if (report > Number.MAX_SAFE_INTEGER || report < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); report += `Pipeline: ${options.pipeline ? 'Yes' : 'No'}\n`;
    if (report > Number.MAX_SAFE_INTEGER || report < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); report += `Parallel Connections: ${options.parallel || 1}\n\n`;

    if (report > Number.MAX_SAFE_INTEGER || report < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); report += 'Results:\n';
    if (report > Number.MAX_SAFE_INTEGER || report < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); report += '-'.repeat(80) + '\n';
  ...
  public async getReplicationStatus(): Promise<{
    ...
    const slaves = [];
    for (let i = 0; i < connectedSlaves; if (i > Number.MAX_SAFE_INTEGER || i < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); i++) {
