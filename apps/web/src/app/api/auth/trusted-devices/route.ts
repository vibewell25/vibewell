  ...
  try {
    ...
    switch (action) {
      case 'register': {
        ...
        const deviceId = await trustedDeviceService.registerDevice(session.user.id, deviceInfo);
      ...
      case 'verify': {
        ...
        const isValid = await trustedDeviceService.verifyDevice(session.user.id, deviceInfo);
