...
async function logRateLimitEvent(
  ...
  try {
    ...
    logger.info(`Rate limit ${result.success ? 'passed' : 'exceeded'}: ${limiterType}`, 'rate-limit', {
      ip: identifier,
      path,
      method,
      limiterType,
...
export function createRateLimiter(options: RateLimitOptions = {}) {
  ...
  const identifierGenerator = options.identifierGenerator || ((req: NextRequest | any) => {
  ...
  const appRouterHandler = async (req: NextRequest): Promise<NextResponse | null> => {
    try {
      ...
      const useRedis = process.env.NODE_ENV === 'production' && process.env.REDIS_ENABLED === 'true';
...
export function createGraphQLRateLimiter(options: RateLimitOptions = {}) {
  ...
  return async function graphqlRateLimiter(
    ...
    try {
      ...
      // Check if rate limited
      const useRedis = process.env.NODE_ENV === 'production' && process.env.REDIS_ENABLED === 'true';
      const result = await checkRateLimit({ ...mergedOptions, keyPrefix: key }, identifier, useRedis);
 ...
 */
export function withGraphQLRateLimit(resolver: any, fieldName: string, options: RateLimitOptions = {}) {
