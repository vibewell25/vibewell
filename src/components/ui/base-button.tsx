...
export const BaseButton = React.forwardRef<HTMLButtonElement, BaseButtonProps>(
  ({ 
    ...
    const LoadingSpinner = () => (
      <svg 
        className={cn("animate-spin -ml-1 mr-2 h-4 w-4", iconClassName)} 
    ...
    return (
      <div className={cn(fullWidth && "w-full", wrapperClassName)}>
        ...
        >
          ...
          {leftIcon && !isLoading && 
            <span className={cn("mr-2", iconClassName)}>{leftIcon}</span>
          }
          {renderContent()}
          {rightIcon && 
            <span className={cn("ml-2", iconClassName)}>{rightIcon}</span>
          }
        </Comp>
      </div>
    );
