 ...
 */
async function collectRateLimitMetrics(): Promise<RateLimitMetrics> {
  try {
 ...
 */
export async function storeMetricsInRedis(metrics: RedisMetrics): Promise<boolean> {
  try {
 ...
 */
export async function getRedisMetricsHistory(limit = 100): Promise<RedisMetrics[]> {
  try {
    const metricsKey = 'vibewell:metrics:redis';
 ...
 */
export async function calculateRedisStats(): Promise<Record<string, any>> {
  try {
