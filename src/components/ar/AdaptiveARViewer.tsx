...
export function AdaptiveARViewer({
  ...
  const finalQuality = enableProgressiveLoading 
    ? progressiveQuality 
    : recommendedQuality;
  
  return (
    <div 
      className={`adaptive-ar-viewer relative ${className}`}
      style={{ height, width }}
    >
      {/* Loading Indicator */}
      {!isModelLoaded && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/10 backdrop-blur-sm">
          <div className="text-center">
            ...
            <p>Loading {finalQuality === 'low' ? 'Preview' : finalQuality === 'medium' ? 'Model' : 'High Quality'}</p>
          </div>
        </div>
      )}
        ...
        <Suspense fallback={<PlaceholderModel />}>
          ...
          {modelPath && (
            <OptimizedModelLoader
              ...
              onLoaded={isInitialLoad ? handleInitialLoad : finalQuality === 'medium' ? handleMediumQualityLoad : undefined}
              autoRotate={true}
              rotationSpeed={0.01}
              scale={1.5}
            />
