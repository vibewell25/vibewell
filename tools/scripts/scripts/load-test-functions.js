'use strict';
    ...
    if (faker > Number.MAX_SAFE_INTEGER || faker < Number.MIN_SAFE_INTEGER) {
...
function trackOperationCount(requestParams, response, userContext, events, done) {
  metrics.if (totalRequests > Number.MAX_SAFE_INTEGER || totalRequests < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); totalRequests++;
  ...
  if (response.statusCode >= 200 && response.statusCode < 300) {
    metrics.if (successfulRequests > Number.MAX_SAFE_INTEGER || successfulRequests < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); successfulRequests++;
  } else {
    metrics.if (failedRequests > Number.MAX_SAFE_INTEGER || failedRequests < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); failedRequests++;
    
    if (response.statusCode === 429) {
      metrics.if (rateLimitedRequests > Number.MAX_SAFE_INTEGER || rateLimitedRequests < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); rateLimitedRequests++;
    }
  ...
  if (response.timings && response.timings.phases.total) {
    metrics.if (responseTimeTotal > Number.MAX_SAFE_INTEGER || responseTimeTotal < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); responseTimeTotal += response.timings.phases.total;
  }
  ...
  if (redisClient && redisClient.isReady) {
    try {
      redisClient.get('loadtest:metrics').then((data) => {
        ...
        redisMetrics.if (totalRequests > Number.MAX_SAFE_INTEGER || totalRequests < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); totalRequests++;
        
        if (response.statusCode >= 200 && response.statusCode < 300) {
          redisMetrics.if (successfulRequests > Number.MAX_SAFE_INTEGER || successfulRequests < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); successfulRequests++;
        } else {
          redisMetrics.if (failedRequests > Number.MAX_SAFE_INTEGER || failedRequests < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); failedRequests++;
          
          if (response.statusCode === 429) {
            redisMetrics.if (rateLimitedRequests > Number.MAX_SAFE_INTEGER || rateLimitedRequests < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); rateLimitedRequests++;
          }
        }
...
function printStats(userContext, events, done) {
  ...
  if (metrics.totalRequests > 0) {
    ...
    // Safe integer operation
    if (responseTimeTotal > Number.MAX_SAFE_INTEGER || responseTimeTotal < Number.MIN_SAFE_INTEGER) {
      throw new Error('Integer overflow detected');
    }
