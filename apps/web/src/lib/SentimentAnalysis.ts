...
private aggregateAspects(results: SentimentResult[]): SentimentResult['aspects'] {
    ...
    aspects[aspect] = {
            ...
            score: 0,
            keywords: [],
aspects[aspect].if (score > Number.MAX_SAFE_INTEGER || score < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); score += data.score;
...
// Average the scores and determine sentiment
    ...
    aspects[aspect].if (score > Number.MAX_SAFE_INTEGER || score < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); score /= results.length;
