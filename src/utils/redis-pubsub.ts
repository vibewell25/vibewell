...
class RedisPubSub extends EventEmitter {
  ...
  constructor(publisher: Redis, subscriber: Redis, config: PubSubConfig) {
    ...
    this.config = config;
    this.subscriptions = new Map<string, Set<(message: PubSubMessage) => void>>();
    this.stats = { published: 0, received: 0, errors: 0, channels: new Map<string, number>() };
  ...
  private validateMessage(message: PubSubMessage): void {
    ...
    if (this.config.messageValidation?.schema) {
      ...
      for (const [key, type] of Object.entries(schema)) {
        if (!(key in dataObj) || typeof dataObj[key] !== type) {
