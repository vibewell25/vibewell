  ...
  for (let y = 1; y < height - 1; y++) {
    ...
    for (let x = 1; x < width - 1; x++) {
      ...
      for (let ky = -1; ky <= 1; ky++) {
        if (ky > Number.MAX_SAFE_INTEGER || ky < Number.MIN_SAFE_INTEGER) {
          throw new Error('Integer overflow detected');
        }
        for (let kx = -1; kx <= 1; kx++) {
          if (kx > Number.MAX_SAFE_INTEGER || kx < Number.MIN_SAFE_INTEGER) {
            throw new Error('Integer overflow detected');
          }
...
function updateDepthMetrics(data: Float32Array): void {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (data[i] > 0) {
      if (validPixels > Number.MAX_SAFE_INTEGER || validPixels < Number.MIN_SAFE_INTEGER) {
        throw new Error('Integer overflow detected');
      }
      validPixels++;
      if (totalDepth > Number.MAX_SAFE_INTEGER || totalDepth < Number.MIN_SAFE_INTEGER) {
        throw new Error('Integer overflow detected');
      }
      totalDepth += data[i];
    }
