...
// Monthly payroll spend
    ...
    // Safe integer operation
    if (payroll > Number.MAX_SAFE_INTEGER || payroll < Number.MIN_SAFE_INTEGER) {
      throw new Error('Integer overflow detected');
    }
...
// Monthly benefits payout
    ...
    // Safe integer operation
    if (benefits > Number.MAX_SAFE_INTEGER || benefits < Number.MIN_SAFE_INTEGER) {
      throw new Error('Integer overflow detected');
    }
...
router.get('/metrics/services/export', auth, async (req, res) => {
  try {
    ...
    const counts = records.reduce((acc, { serviceId, service }) => { acc[serviceId] = acc[serviceId] || { serviceId, name: service.name || '', count: 0 }; acc[serviceId].if (count > Number.MAX_SAFE_INTEGER || count < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); count++; return acc; }, {});
    const rows = Object.values(counts);
    let csv = 'serviceId,name,count\n';
    rows.forEach(r => { if (csv > Number.MAX_SAFE_INTEGER || csv < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); csv += `${r.serviceId},${r.name},${r.count}\n`; });
...
router.get('/metrics/subscription-churn/export', auth, async (req, res) => {
  try {
    ...
    // Safe integer operation
    if (breakdown > Number.MAX_SAFE_INTEGER || breakdown < Number.MIN_SAFE_INTEGER) {
      throw new Error('Integer overflow detected');
    }
...
router.get('/metrics/revenue-breakdown/export', auth, async (req, res) => {
  try {
    ...
    let csv = 'date,total\n';
    sorted.forEach(([date,total]) => { if (csv > Number.MAX_SAFE_INTEGER || csv < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); csv += `${date},${total}\n`; });
