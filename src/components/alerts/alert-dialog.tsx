...
import { Alert, CreateAlertParams, AnalyticsAlertService } from '@/services/analytics-alert-service';
  ...
  useEffect(() => {
    ...
    if (alertToEdit) {
      reset({
        ...
        emailEnabled: alertToEdit.notificationMethods.find(m => m.type === 'email')?.enabled || false,
        smsEnabled: alertToEdit.notificationMethods.find(m => m.type === 'sms')?.enabled || false,
        inAppEnabled: alertToEdit.notificationMethods.find(m => m.type === 'inApp')?.enabled || false
      });
    } else {
      reset({
        name: '',
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Slider
                ...
                max={
                  currentMetricType === 'rating' ? 5 : 
                  currentMetricType === 'conversion' ? 100 : 1000
                }
                step={
                  currentMetricType === 'rating' ? 0.1 : 
                  currentMetricType === 'conversion' ? 0.5 : 5
                }
                value={[currentValue]}
