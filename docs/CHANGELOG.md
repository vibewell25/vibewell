...
<!-- From ./node_modules/tryer/CHANGES.md -->
# Changes
...
<!-- From ./node_modules/asap/CHANGES.md -->
...
[Mr]: https://github.com/montagejs/m
...
<!-- From ./node_modules/chrome-trace-event/CHANGES.md -->
...
<!-- From ./node_modules/@expo/bunyan/CHANGES.md -->
# bunyan Changelog
...
- [pull #318] Add `reemitErrorEvents` optional boolean for streams added to a
  ...
  behaviour is as follows:
    ...
    - `reemitErrorEvents: true`: error events will be re-emitted on the Logge
  ...
  that a (presumably rare) situation where code adds dozens of Bunyan Logge
...
- [issue #100] Gracefully handle the case of an unbound
  ...
  Before this change, bunyan would throw. Now it emits a warning to stder
...
- [issue #178, #181] Get at least dtrace-provider 0.3.1 fo
...
- [pull #168] Fix handling of `log.info(err)` to use the `log` Logger's `err`
  serializer if it has one, instead of always using the core Bunyan er
...
- [Slight backward incompatibility] Change the default error serialization
  ...
  will log these attributes: message, name, stack, code, signal. The latte
...
- [issue #56] Support `bunyan -p NAME` to dtrace all PIDs matching 'NAME' in
  ...
  This is useful for usage of node's [cluste
...
- Export `bunyan.resolveLevel(NAME-OR-NUM)` to resolve a level name or numbe
...
- [pull #29] Add -l/--level for level filtering, and -c/--condition fo
  arbitrary conditional filtering (by github.com/isaacs):

        $ ... | bunyan -l error   # filter out log records below erro
...
- [pull #15] A new `bunyan.RingBuffer` stream class that is useful fo
...
- Add support for "raw" streams. This is a logging stream that is given
  ...
  See "examples/raw-stream.js". I expect raw streams to be useful fo
...
- Add `log.info(err)` special case for logging an `Error` instance. Fo
...
<!-- From ./server/node_modules/asap/CHANGES.md -->
