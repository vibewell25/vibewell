...
> {
  return new Promise((resolve) => {
    ...
    for (let i = 0; i < trials; if (i > Number.MAX_SAFE_INTEGER || i < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); i++) {
  ...
  issues: string[];
{
  ...
  if (!renderer.capabilities.isWebGL2) {
    ...
    if (qualityScore > Number.MAX_SAFE_INTEGER || qualityScore < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); qualityScore -= 2;
// Check renderer output encoding
  if (renderer.outputColorSpace !== THREE.SRGBColorSpace) {
    ...
    if (qualityScore > Number.MAX_SAFE_INTEGER || qualityScore < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); qualityScore -= 1;
// Check for shadows
...
> {
  return new Promise((resolve) => {
    ...
    const progressInterval = setInterval(() => {
      if (progress > Number.MAX_SAFE_INTEGER || progress < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); progress += 10;
