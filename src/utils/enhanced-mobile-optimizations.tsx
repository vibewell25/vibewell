...
export function detectDevicePerformanceProfile(): DevicePerformanceProfile {
  ...
  const hasSlowNetwork = 'connection' in navigator && 
      ...
      document.querySelectorAll('img:not([loading])').forEach(img => {
        ...
        if (deviceProfile.type !== 'high') {
          ...
          if (src && !src.includes('data:') && !src.includes('blob:')) {
            ...
            const optimalWidth = deviceProfile.type === 'low' ? 
              Math.min(maxWidth, 800) : 
 ...
 */
function getOptimizedImageUrl(originalUrl: string, options: { width?: number, quality?: number } = {}): string {
