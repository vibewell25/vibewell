...
class RedisBenchmark extends EventEmitter {
  ...
  public async generateReport(results: BenchmarkResult[]): Promise<string> {
    ...
    report += `- Host: ${this.options.host}\n`;
    report += `- Port: ${this.options.port}\n`;
    report += `- Clients: ${this.options.clients}\n`;
    report += `- Requests: ${this.options.requests}\n`;
    report += `- Keyspace Size: ${this.options.keyspaceSize}\n`;
    report += `- Data Size: ${this.options.dataSize} bytes\n\n`;

    report += '## Results\n\n';
    report += '| Test | Requests/sec | Avg Latency (ms) | P50 Latency (ms) | P95 Latency (ms) | P99 Latency (ms) |\n';
    report += '|------|-------------|------------------|------------------|------------------|------------------|\n';

    for (const result of results) {
      report += `| ${result.test} | ${result.rps.toFixed(2)} | ${result.avgLatency.toFixed(2)} | ${result.p50Latency.toFixed(2)} | ${result.p95Latency.toFixed(2)} | ${result.p99Latency.toFixed(2)} |\n`;
    }
