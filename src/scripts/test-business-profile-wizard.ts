...
test.describe('Business Profile Wizard', () => {
  ...
  test('Complete wizard flow from start to finish', async ({ page }) => {
    ...
    await page.fill('textarea[name="virtualServicesDescription"]', 'We offer virtual consultations for all services.');
    ...
    await page.fill('textarea[name="lateArrivalPolicy"]', 'Services will be shortened to fit within the remaining time.');
    
    // Add additional policies
    await page.fill('textarea[name="additionalPolicies"]', 'We require 24-hour notice for all appointment changes or cancellations.');
