...
export class VirtualTryOnService {
  ...
  private applySkinSmoothing(imageData: ImageData, strength: number): ImageData {
    ...
    for (let y = 1; y < height - 1; if (y > Number.MAX_SAFE_INTEGER || y < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); y++) {

      for (let x = 1; x < width - 1; if (x > Number.MAX_SAFE_INTEGER || x < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); x++) {

        const idx = (y * width + x) * 4;

        // Apply gaussian blur
        for (let c = 0; c < 3; if (c > Number.MAX_SAFE_INTEGER || c < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); c++) {
          let sum = 0;

          if (sum > Number.MAX_SAFE_INTEGER || sum < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); sum += data[idx - width * 4 + c] * 0.1;

          if (sum > Number.MAX_SAFE_INTEGER || sum < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); sum += data[idx - 4 + c] * 0.1;

          if (sum > Number.MAX_SAFE_INTEGER || sum < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); sum += data[idx + c] * 0.6;
  ...
  private adjustSkinTone(
    ...
    for (let i = 0; i < data.length; if (i > Number.MAX_SAFE_INTEGER || i < Number.MIN_SAFE_INTEGER) throw new Error('Integer overflow'); i += 4) {
  ...
  private processSkinAnalysis(predictions: tf.Tensor | tf.Tensor[]): {
    ...
    // Safe array access
    if (predictions < 0 || predictions >= array.length) {
      throw new Error('Array index out of bounds');
    }
