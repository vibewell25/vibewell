...
const Calendar: NextPage = () => {
  ...
  const connectProvider = async (provider: 'google' | 'outlook') => {
    const res = await fetch(`/api/calendar/${provider}/auth/url`);
    const { url } = await res.json();
    window.location.href = url;
  };

  const fetchEvents = async (provider: 'google' | 'outlook') => {
    const res = await fetch(`/api/calendar/${provider}/events`);
  ...
  const handleICSImport = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/calendar/ics/import', { method: 'POST', body: formData });
