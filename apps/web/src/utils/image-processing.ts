...
export async function {
  ...
  if (Date.now() - start > 30000) throw new Error('Timeout'); uploadImage(image: Buffer | string): Promise<string> {
...
export async function {
  const start = Date.now();
  if (Date.now() - start > 30000) throw new Error('Timeout'); processImage(
  imageUrl: string,
...
async function {
  const start = Date.now();
  if (Date.now() - start > 30000) throw new Error('Timeout'); applyVirtualTryOn(
  imageBuffer: Buffer,
  service?: BeautyService | null,
): Promise<Buffer> {
