...
const providers: Provider[] = [
  { id: 'provider1', name: 'Sarah Johnson', avatar: '/providers/sarah.jpg', rating: 4.8, specialties: ['Hair Styling', 'Color'], availableSlots: ['10:00 AM', '11:30 AM', '2:00 PM', '3:30 PM'] },
  // Add more providers...
];
    ...
    <Layout>
      <div className="container-app py-12">
        <div className="mb-8">
          ...
          <p className="text-xl text-muted-foreground">Choose your service and schedule with our expert providers</p>
        </div>
        <div className="mb-8">
          <div className="flex items-center justify-between">
            {[1, 2, 3, 4].map((s) => (
              <div key={s} className="flex items-center">
                <div className={`flex h-8 w-8 items-center justify-center rounded-full ${s <= step ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}>{s}</div>
                {s < 4 && <div className={`mx-2 h-1 w-16 ${s < step ? 'bg-primary' : 'bg-muted'}`} />}
              </div>
            ))}
          </div>
          <div className="mt-2 flex justify-between text-sm text-muted-foreground">
            <span>Service</span><span>Provider</span><span>Time</span><span>Payment</span>
          </div>
        </div>
        <div className="space-y-8">
          ...
          {step === 2 && (
            <Card>
              ...
              <CardContent>
                <div className="grid gap-4">
                  {providers.map((p) => (
                    <Card key={p.id} className={`cursor-pointer transition-all ${selectedProvider?.id === p.id ? 'border-primary' : ''}`} onClick={() => setSelectedProvider(p)}>
                      <CardContent className="p-4">
              ...
              <CardContent>
                <div className="grid gap-6 md:grid-cols-2">
                  <div>
                    ...
                    <Calendar mode="single" selected={selectedDate} onSelect={setSelectedDate} className="mt-2 rounded-md border" />
                  </div>
                  <div>
              ...
              <CardContent>
                <div className="space-y-6">
                  <div>
                    ...
                    <RadioGroup value={paymentMethod} onValueChange={setPaymentMethod} className="mt-2 grid gap-4">
                  ...
                  <div className="border-t pt-4">
                    <div className="mb-2 flex justify-between">
                      <span>Service</span><span>${selectedService?.price}</span>
                    </div>
          ...
          <div className="flex justify-between">
            ...
            <Button onClick={handleNext} disabled={(step === 1 && !selectedService) || (step === 2 && !selectedProvider) || (step === 3 && (!selectedDate || !selectedTime))}>
              {step === 4 ? (
                <><CheckCircleIcon className="mr-2 h-5 w-5" />Confirm Booking</>
              ) : (
                <><span>Next</span><ArrowRightIcon className="ml-2 h-5 w-5" /></>
              )}
            </Button>
          </div>
        </div>
